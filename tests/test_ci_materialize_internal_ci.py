...
def test_startrek_release_stage_excludes_memory_docs_and_preserves_public_readme() -> None:
    ...
    assert "docs/" in cli["include"]
    assert "specs/" not in cli["include"]
    assert "specs/" in cli["exclude"]
    assert "build.rs" in agent["include"]
    assert "docs/" in agent["include"]
    assert "specs/" not in agent["include"]
    assert "specs/" in agent["exclude"]
    ...
    assert "docker buildx build" in startrek_release
    assert "--output \"type=docker,dest=${ARTIFACT}\"" in startrek_release
    assert "manifest.json" in startrek_release
